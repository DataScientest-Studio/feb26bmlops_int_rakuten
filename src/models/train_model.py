...
def train_text_linear_svm_model(cfg):
    return train_text_linear_svm_from_csv(cfg)
...
def train_model(cfg):
    ...
    if model_type == "text_linear_svm":
        return train_text_linear_svm_model(cfg)
    ...
    raise ValueError("model_type must be one of: text, text_linear_svm, image, fusion")
...
if __name__ == "__main__":
    ...
    # for linear SVM, use this config instead:
    CFG = {
        "experiment_id": datetime.now().strftime("%Y%m%d_%H%M%S"),
        "text_col": "text_stripped",
        "label_col": "prdtypecode",
